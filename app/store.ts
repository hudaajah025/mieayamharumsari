...
const useStore = create<StoreState>((set, get) => ({
  ...
  updatePassword: async (oldPassword: string, newPassword: string) => {
    try {
      ...
      await database.updateUserPassword(user.id, newPassword);

      // Update user state dengan password baru
      set({ 
        user: { 
          ...user, 
  ...
  updateProfile: async (userData: Partial<UserData>) => {
    try {
      ...
      const updatedUser = await database.updateProfile(currentUser.id, userData);
